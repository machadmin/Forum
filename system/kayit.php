...
if(!$_SESSION){
	if($_POST){
		...
		if(!$isim || !$sifre || !$email){
			...
			echo '<div class="hata">Bu mail adresi geçersiz, başka bir tane dene...</div>';
		}else{
			$sifre = md5($sifre);
			...
			if($y){
				echo '<div class="hata"><span style="color:red;">'.$x["uye_adi"].'</span> isimli üye sistemde kayıtlı, başka bir isim dene...</div>';
			}else{
				...
				if($kayit){
					echo '<div class="basarili2">Başarıyla kayıt oldunuz, üyeliğiniz ile giriş yapabilirsiniz...</div>';
